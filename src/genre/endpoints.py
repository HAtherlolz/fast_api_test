...
from .schema import Genre, Genre_Pydantic, GenreIn_Pydantic


genre_router = APIRouter()


@genre_router.post("/genre/", response_model=Genre_Pydantic)
async def create_genre(genre: GenreIn_Pydantic):
    """ Create a song genre """
    return await Genre.create(**genre.dict())


@genre_router.get("/genre/", response_model=list[Genre_Pydantic])
async def list():
    """ Get all genres """
    return await Genre.all()


@genre_router.get("/genre/{genre_id}", response_model=Genre_Pydantic)
async def retrieve(genre_id: int):
    """ Get target genre """
    return await Genre.get(id=genre_id)


@genre_router.put("/genre/{genre_id}")
